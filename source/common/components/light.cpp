...
namespace our
{
    ...
    void LightComponent::deserialize(const nlohmann::json &data)
    {
        ...
        // Support both array format [constant, linear, quadratic] and individual fields
        if (data.contains("attenuation") && data["attenuation"].is_array() && data["attenuation"].size() == 3)
        {
            attenConstant = data["attenuation"][0].get<float>();
            attenLinear = data["attenuation"][1].get<float>();
            attenQuadratic = data["attenuation"][2].get<float>();
        }
        else
        {
