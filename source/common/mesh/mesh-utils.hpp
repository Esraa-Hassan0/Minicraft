...
namespace our::mesh_utils {
    ...
    MeshBuildData buildChunkMeshData(const voxel::Chunk& chunk, const voxel::World& world, int blockTypeFilter = -1);

    // Convenience wrappers that allocate/update Mesh objects from chunk data.
    Mesh* buildChunkMesh(const voxel::Chunk& chunk, const voxel::World& world, int blockTypeFilter = -1);
    void updateChunkMesh(Mesh* mesh, const voxel::Chunk& chunk, const voxel::World& world, int blockTypeFilter = -1);
}