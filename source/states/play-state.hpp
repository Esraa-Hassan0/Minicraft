...
class Playstate : public our::State {
    ...
    void onDraw(double deltaTime) override {
        ...
        // Level Target Popup Notification (from main branch)
        ...
        // Check for level target updates
        if (currentPlayer && currentPlayer->level > currentLevelTarget) {
            currentLevelTarget = currentPlayer->level;
        ...
        if (levelTargetPopupTime > 0.0f) levelTargetPopupTime -= (float)deltaTime;
